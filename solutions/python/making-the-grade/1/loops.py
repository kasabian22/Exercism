...
def round_scores(student_scores):
    ...
    rounded_list = [round(score) for score in student_scores]
    rounded_list.reverse()
    return rounded_list
...
def letter_grades(highest):
    ...
    step = (highest - 40) / 4
    grades = [grade for grade in range(41, highest, round(step))]
    return grades
...
def student_ranking(student_scores, student_names):
    ...
    """
    final_list = []
    for index, (student, score) in enumerate(zip(student_names, student_scores), 1):
        final_list.append(f"{index}. {student}: {score}")
    return final_list
