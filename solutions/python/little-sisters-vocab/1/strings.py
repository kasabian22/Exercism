...
def remove_suffix_ness(word):
    ...
    """
    suffix_ness = word[-4:]
    replace_ness = word.replace(suffix_ness, "")
    if replace_ness[-1] == "i":
        return replace_ness.replace(replace_ness[-1], "y")
    return replace_ness
...
def adjective_to_verb(sentence, index):
    ...
    """
    sentence_split = sentence.split()[index]
    if sentence_split[-1] == ".":
        return sentence_split.replace(sentence_split[-1], "") + "en"
    return sentence_split + "en"
