...
def approx_average_is_average(hand):
    ...
    if (hand[0] + hand[-1]) / 2 == card_average(hand) or hand[int((len(hand) + 1) / 2) -1] == card_average(hand):
        return True
    return False
...
def average_even_is_average_odd(hand):
    ...
    """
    odd_indexes = [number for index, number in enumerate(hand) if index % 2 == 1]
    even_indexes = [number for index, number in enumerate(hand) if index % 2 == 0]
    if sum(odd_indexes) / len(odd_indexes) == sum(even_indexes) / len(even_indexes):
        return True
    return False
